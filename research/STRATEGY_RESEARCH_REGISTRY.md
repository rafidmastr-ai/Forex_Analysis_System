...
- **Our own tested result**: ran the full Train/Validation/OOS/perturbation/ablation/cross-symbol pipeline (`scripts/optimize_strategy.py sweep_displacement`, `min_confidence=65`, same EURUSD split as Classic/SMC/ICT). The equal-weight prior was badly lossy (Train obj -45.99, net R -61.19, 123 resolved); a searched candidate weighting mostly `displacement_strength` (0.439) and `retracement_depth` (0.353), with only 3% on `sweep_quality` and 18% on `premium_discount_depth`, improved Train to -14.65, Validation to -1.60 (vs -6.91 baseline), OOS to -4.35 (vs -24.89 baseline), and — critically — did NOT collapse under weight-redistribution perturbation (worst-case drop of 1.0 against a 3.2 margin). XAUUSD cross-check: positive on only 1 of 3 phases (Validation +3.38; Train -5.43, OOS -5.00). **Classified Robust Candidate** and adopted as the strategy's new `DEFAULT_WEIGHTS` (§2) — but OOS net R is still -9.00R over 9 trades: this is "more defensible than an arbitrary guess," not "profitable."
- **Status**: **Robust Candidate** (adopted, wired into `core/strategies/bootstrap.py`) — see §2.
...
- **Our own tested result**: same pipeline as above. The equal-weight prior was actually net-positive on raw R (Train net R +18.50) but had a poor composite objective (-4.45) from drawdown/instability; the searched candidate concentrated 100% of weight on `range_quality` (a pure simplex vertex — a red flag in itself, see the perturbation-methodology note below), improving Train to +4.07 and Validation to +4.67 (vs -9.81 baseline) — the only STRATEGY-LEVEL positive Train/Validation objective pair found anywhere in this project. OOS stayed negative (-3.15 on only 7 resolved trades, though still better than baseline's -9.31). Perturbation (after the redistribution-based fix, see below): mild degradation only (worst-case -1.13 against a 9.33 margin) — genuinely stable, not the false "zero effect" first observed. **XAUUSD cross-check: 0 resolved trades in all 3 phases** — traced to a real, non-buggy cause (see below), not evidence of failure so much as evidence of no evidence. **Classified `Candidate`, not `Robust Candidate`** (fails the cross-symbol-generalization bar) — `DEFAULT_WEIGHTS` left at the original equal-split prior; the tested candidate is not adopted.
- **Methodology bug found and fixed while running this exact experiment**: the first run's perturbation test showed literally zero effect across all 30 runs — traced to `ComponentWeights.perturbed()` (rescale-and-renormalize) being a mathematical no-op on a pure vertex (scaling the only nonzero weight and renormalizing always returns 1.0). Fixed by adding `ComponentWeights.perturbed_toward()` (weight redistribution between components), which the driver now uses exclusively; Classic/SMC/ICT/sweep_displacement's earlier results were unaffected (none of their candidates were pure vertices) but session_breakout was rerun. See `core/confidence/component_scoring.py` and its tests.
- **XAUUSD zero-trade diagnosis**: directly probed `SessionBreakoutStrategy.analyze()` against 3 months of raw XAUUSD M15 data outside the backtest engine — it DOES fire (132 raw signals), but with 100% weight on `range_quality`, confidence rarely reaches the fixed 65 threshold on gold (only ~2 of 132 signals scored >=65 in that sample) because XAUUSD's typical Asian-session range relative to its own ATR differs structurally from EURUSD's. This is a real symbol-dependence finding, not a bug — and exactly why the acceptance rule would not label this a Robust Candidate without evidence it generalizes.
- **Status**: Implemented, tested — **Candidate** (not Robust Candidate; not adopted) — see §2.
